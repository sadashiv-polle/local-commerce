...
## Owner products, prices and stock

The owner workspace now includes product editing, dedicated selling prices,
warehouse/account setup, ERPNext stock additions and removals, manual sold-out
controls, archive/restore, low-stock indicators and adjustment history.

See [owner workflow and setup](docs/owner-inventory.md) for exact operation,
accounting, permission and retry behavior, and [implementation status](IMPLEMENTATION_STATUS.md)
for the full remaining scope. This update requires migration before using the new
UI. New ERPNext integration tests are committed but unrun locally; use a test site
before relying on the workflow for live stock.
