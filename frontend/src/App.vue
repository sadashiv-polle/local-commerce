...
async function loadHeaderAddress(preferred = '') {
  ...
  try {
    ...
    if (!remembered) try { remembered = sessionStorage.getItem(`lc-selected-address:${session.value.user}`) || '' } catch { /* Leave the address unselected. */ }
    headerAddress.value = headerAddresses.value.find(address => address.name === remembered) || null
...
function chooseHeaderAddress(address) {
  ...
  try { sessionStorage.setItem(`lc-selected-address:${session.value.user}`, address.name) } catch { /* Selection still applies now. */ }
...
<template>
  <div class="commerce-app" :class="{ 'admin-view': adminView, 'owner-view': ownerView, 'delivery-view': deliveryView, 'customer-view': !ownerView && !deliveryView, 'has-global-cart': savedCartLines && !adminView }">
    <header class="topbar">
