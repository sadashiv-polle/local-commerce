...
async function save() {
  ...
  try {
    const s = shop.value
    const values = { shop: s.name, shop_name: s.shop_name, status: s.status, description: s.description || '', order_response_minutes: s.order_response_minutes || 10, accepting_orders: s.accepting_orders ? 1 : 0, opening_hours: s.opening_hours, minimum_order_amount: s.minimum_order_amount || 0, free_delivery_above: s.free_delivery_above || 0, delivery_fee_per_km: s.delivery_fee_per_km || 0, delivery_included_km: s.delivery_included_km || 0, delivery_fee: s.delivery_fee || 0 }
      ...
      <template v-if="shop">
        ...
        <form v-show="tab === 'settings'" class="lc-form" @submit.prevent="save">
          ...
          <fieldset :disabled="!canEdit || saving" class="workspace-fields">
            ...
            <section class="shop-hours"><span class="eyebrow">DELIVERY PRICING</span><h3>Order minimum &amp; delivery fees</h3><div class="form-columns"><label>Minimum item subtotal<input v-model.number="shop.minimum_order_amount" type="number" min="0" step="0.01"><small>0 means no minimum.</small></label><label>Free delivery above<input v-model.number="shop.free_delivery_above" type="number" min="0" step="0.01"><small>0 disables free delivery.</small></label><label>Base delivery fee<input v-model.number="shop.delivery_fee" type="number" min="0" step="0.01"></label><label>Distance included (km)<input v-model.number="shop.delivery_included_km" type="number" min="0" step="0.1"></label><label>Fee per additional km<input v-model.number="shop.delivery_fee_per_km" type="number" min="0" step="0.01"><small>Uses straight-line distance. 0 keeps a flat fee.</small></label></div></section>
