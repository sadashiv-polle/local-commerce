...
function useLocation() {
  error.value = ''
  if (!navigator.geolocation) { error.value = 'Location is unavailable. Enter coordinates or tap the map.'; return }
  locating.value = true
  navigator.geolocation.getCurrentPosition(
    position => { pickLocation(position.coords); locating.value = false },
    () => { error.value = window.isSecureContext ? 'Location access failed. Allow it or enter coordinates.' : 'Automatic location needs HTTPS. Enter coordinates or tap the map.'; locating.value = false },
      ...
      <template v-if="data">
        <section class="address-book">
          ...
          <div v-if="data.addresses.length" class="address-grid">
            <article v-for="address in data.addresses" :key="address.name" class="address-card" :class="{ default: address.is_default }"><header><span class="address-icon" aria-hidden="true">{{ address.address_type === 'Home' ? '⌂' : address.address_type === 'Work' ? '▦' : '⌖' }}</span><div><span class="eyebrow">{{ address.address_type }}</span><h3>{{ address.address_label }}</h3></div><span v-if="address.is_default" class="status-pill">Selected</span></header><strong>{{ address.recipient }}</strong><p>{{ address.line1 }}<br>{{ address.city }} · {{ address.postal_code }}</p><small>{{ address.phone }} · {{ Number(address.latitude).toFixed(5) }}, {{ Number(address.longitude).toFixed(5) }}</small><footer><button :disabled="busy" @click="editAddress(address)">Edit</button><button v-if="!address.is_default" :disabled="busy" @click="setDefault(address)">Use for delivery</button><button class="text-danger" :disabled="busy" @click="archiveAddress(address)">Remove</button></footer></article>
          </div>
