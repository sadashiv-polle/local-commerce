...
const props = defineProps({ slotName: { type: String, required: true }, manageable: Boolean })
const emit = defineEmits(['open-order', 'changed'])
const data = ref(null), error = ref(''), busy = ref(false)
const points = computed(() => data.value ? [{ ...data.value.origin, kind: 'shop', label: 'Pickup' }, ...data.value.stops.map((s, i) => ({ ...s.destination_location, kind: 'customer', label: `${i + 1}. ${s.recipient}` }))] : [])
