...
const hasEstimatedWeights = computed(() => Object.values(cart.value).some(row => row.option_id))
    ...
    <template v-if="catalog">
      ...
      <div v-if="cartOpen" class="cart-backdrop" @click.self="closeCart">
        <aside class="cart-drawer" role="dialog" aria-modal="true" aria-labelledby="cart-title">
          ...
          <p v-if="hasEstimatedWeights" class="packed-weight-notice">Prices for count and weight options are estimates. The shop weighs your packed order and confirms the final bill before dispatch.</p><div class="cart-line-list">
            <article v-for="item in cart" :key="cartKey(item)" class="cart-line"><div class="cart-line-art"><img v-if="item.image" :src="item.image" :alt="item.item_name" loading="lazy" decoding="async" @error="item.image = ''"><span v-else aria-hidden="true">{{ item.item_name.slice(0, 1).toUpperCase() }}</span></div><div><strong>{{ item.item_name }}</strong><small>{{ item.option_label || item.uom }}</small><small>{{ item.option_id ? '≈ ' : '' }}{{ money(item.rate) }} / {{ item.option_id ? 'pack' : item.uom }}</small></div><div class="quantity-stepper"><button type="button" :disabled="busy || !!pending" :aria-label="`Remove one ${item.item_name}`" @click="updateQuantity(item, -1)">−</button><strong>{{ item.quantity }}</strong><button type="button" :disabled="busy || !!pending || item.quantity >= item.available" :aria-label="`Add one ${item.item_name}`" @click="updateQuantity(item, 1)">+</button></div><strong>{{ money(item.rate * item.quantity) }}</strong></article>
          </div>
