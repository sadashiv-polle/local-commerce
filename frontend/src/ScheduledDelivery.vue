...
function edit(row) { reuseSource.value = null; editing.value = row.name; form.value = { ...row }; for (const key of ['ordering_start', 'ordering_end', 'delivery_start', 'delivery_end']) form.value[key] = String(row[key]).replace(' ', 'T').slice(0, 16); selectedItems.value = (row.products || '').split('\n').filter(Boolean) }
async function startReuse(row) {
  reuseSource.value = row; reuseDate.value = tomorrowInZone(settings.value.timezone)
...
async function toggle(row, enabled) { await run(() => call('scheduled.save_slot', { shop: props.shop, name: row.name, values: { enabled: enabled ? 1 : 0 } }, true), enabled ? 'Slot shown to customers when its dates are eligible.' : 'Slot hidden from customers. Existing orders are unchanged.') }
async function save() {
  ...
  <section class="scheduled-panel">
    ...
    <template v-if="settings">
      ...
      <form v-if="session.platform_admin" ref="editor" class="scheduled-editor" @submit.prevent="save">
        <h4>{{ reuseSource ? 'Reuse slot for another day' : editing ? 'Edit delivery slot' : 'Create delivery slot' }}</h4><p>One slot forms one shop delivery batch. After the first booking, timing and area are locked to protect customer commitments.</p>
        <label v-if="reuseSource">New delivery date<input v-model="reuseDate" type="date" required @change="applyReuse"><small>A new slot will be saved. Past orders and their original dates stay unchanged.</small></label><fieldset :disabled="busy">
