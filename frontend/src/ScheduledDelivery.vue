...
const props = defineProps({ shop: { type: String, required: true }, batchesOnly: Boolean })
const emit = defineEmits(['changed'])
const revision = ref(0), showArchived = ref(false)
const visibleSlots = computed(() => (settings.value?.slots || []).filter(slot => props.batchesOnly ? slot.order_count > 0 : showArchived.value || !slot.archived))
...
async function load() {
  try { settings.value = await call('scheduled.settings', { shop: props.shop, start: pageStart.value }); drivers.value = await call('orders.drivers', { shop: props.shop }); revision.value++ }
...
async function confirmDelete() {
  await run(async () => { await call(deleting.value.daily ? 'scheduled.delete_schedule' : 'scheduled.delete_slot', { name: deleting.value.row.name }, true); deleting.value = null; editing.value = ''; form.value = blank(); selectedItems.value = [] }, 'Schedule removed. Existing order history is preserved.')
}
...
let refreshTimer
async function batchChanged() { await load(); emit('changed') }
onMounted(() => { load(); if (props.batchesOnly) refreshTimer = window.setInterval(load, 20000) })
onBeforeUnmount(() => window.clearInterval(refreshTimer))
</script>
<template>
  <section class="scheduled-panel">
    <div class="workspace-heading"><div><span class="eyebrow">{{ batchesOnly ? 'SCHEDULED ORDERS' : 'DELIVERY BOOKINGS' }}</span><h3>{{ batchesOnly ? 'Batch preparation & delivery' : 'Normal & scheduled delivery' }}</h3></div><button v-if="batchesOnly" type="button" @click="load">Refresh batches</button></div>
    <p v-if="error" role="alert" class="lc-error">{{ error }}</p><p v-if="message" role="status">{{ message }}</p>
    <section v-if="deleting" ref="deletePanel" class="schedule-delete-confirm" role="alertdialog" aria-label="Confirm schedule deletion">
      <h4>Delete {{ deleting.row.title }}?</h4><p>This stops future bookings for this schedule. Existing orders and their delivery batches stay available.</p>
      <div class="button-row"><button :disabled="busy" @click="deleting = null">Keep schedule</button><button class="schedule-delete" :disabled="busy" @click="confirmDelete">{{ busy ? 'Deleting…' : 'Yes, delete schedule' }}</button></div>
    </section>
    <template v-if="settings">
      <p>Times use {{ settings.timezone }}. Scheduled delivery has no delivery fee.</p>
      <template v-if="!batchesOnly">
        <div class="form-columns"><label class="check-label"><input v-model="settings.normal" type="checkbox" :disabled="!canManage">Normal delivery</label><label class="check-label"><input v-model="settings.scheduled" type="checkbox" :disabled="!canManage">Scheduled delivery</label></div>
        <button v-if="canManage" type="button" :disabled="busy" @click="run(() => call('scheduled.configure', { shop, normal: settings.normal ? 1 : 0, scheduled_enabled: settings.scheduled ? 1 : 0 }, true), 'Delivery options saved.')">Save delivery options</button>
        <div class="schedule-list-heading"><h4>Daily time slots</h4><span>Repeat automatically · show or hide anytime</span></div>
        <p v-if="!settings.schedules?.length">Create your first daily slot below. No daily date changes needed.</p>
        <article v-for="schedule in settings.schedules" :key="schedule.name" class="scheduled-slot">
          ...
          <label v-if="canManage" class="check-label"><input type="checkbox" :checked="!!schedule.enabled" :disabled="busy" @change="toggle(schedule, $event.target.checked, true)">Show to customers every day</label>
          <div class="button-row"><button v-if="canManage" type="button" @click="edit(schedule, true)">Edit times &amp; settings</button><button v-if="canManage" class="schedule-delete" type="button" @click="requestDelete(schedule, true)">Delete schedule</button></div>
        </article>
        <form v-if="canManage" ref="editor" class="scheduled-editor" @submit.prevent="save">
      ...
      <h4 v-if="!batchesOnly">Dated batches &amp; history</h4><label v-if="!batchesOnly" class="check-label"><input v-model="showArchived" type="checkbox">Show removed batches</label>
      <p v-if="batchesOnly && !visibleSlots.length">No booked scheduled batches on this page. Accept scheduled requests below; their batches appear here automatically.</p>
      <article v-for="slot in visibleSlots" :key="slot.name" class="scheduled-slot"><h4>{{ slot.title }} <small v-if="slot.past">· Past slot</small></h4><label v-if="!batchesOnly && canManage && !slot.daily_schedule" class="check-label"><input :checked="!!slot.enabled" type="checkbox" :disabled="busy" @change="toggle(slot, $event.target.checked)">Show to customers</label><p v-if="slot.past && !batchesOnly" class="muted">Kept for reuse and history. Use “Reuse for another day” to offer this slot again.</p><p>Order: {{ slot.ordering_start }} – {{ slot.ordering_end }}</p><p>Delivery: {{ slot.delivery_start }} – {{ slot.delivery_end }}</p><p v-if="slot.archived" class="muted">Removed from schedule settings. Order history is retained.</p><p>{{ slot.order_count }} / {{ slot.capacity }} orders · {{ slot.compiled ? 'Compiled' : 'Awaiting ordering cutoff' }}</p><div class="button-row"><button v-if="!batchesOnly && canManage && !slot.has_bookings && !slot.daily_schedule" type="button" @click="edit(slot)">Edit slot</button><button v-if="!batchesOnly && canManage && !slot.daily_schedule" type="button" @click="startReuse(slot)">Reuse for another day</button><button v-if="!batchesOnly && canManage && !slot.has_bookings && !slot.daily_schedule" type="button" class="schedule-delete" @click="requestDelete(slot, false)">Delete slot</button><button v-if="!batchesOnly && canManage && slot.has_bookings" type="button" :disabled="busy" @click="run(() => call('scheduled.archive_slot', { name: slot.name, archived: slot.archived ? 0 : 1 }, true), slot.archived ? 'Batch restored to the list. Bookings remain closed.' : 'Removed from settings. Existing orders remain available in Orders.')">{{ slot.archived ? 'Restore to list' : 'Remove from this list' }}</button><button v-if="!batchesOnly" type="button" @click="activeBatch = activeBatch === slot.name ? '' : slot.name">{{ activeBatch === slot.name ? 'Hide batch controls' : 'Manage batch' }}</button><button type="button" @click="routeSlot = slot.name">View batch route</button></div><BatchActions v-if="batchesOnly || activeBatch === slot.name" :slot-name="slot.name" :revision="revision" @changed="batchChanged" /><div v-if="batchesOnly || activeBatch === slot.name" class="button-row"><select v-model="rider"><option value="">Choose delivery person</option><option v-for="driver in drivers" :key="driver.user" :value="driver.user">{{ driver.full_name }}</option></select><button type="button" :disabled="busy || !rider" @click="run(() => call('scheduled.assign', { slot: slot.name, delivery_user: rider }, true), 'Batch assigned.')">Assign ready batch</button></div></article>
