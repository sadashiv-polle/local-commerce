...
function money(item) { return displayPrice(item).rate == null ? 'Unavailable' : (displayPrice(item).from ? 'From ' : '') + new Intl.NumberFormat(undefined, { style: 'currency', currency: item.currency }).format(displayPrice(item).rate) }
