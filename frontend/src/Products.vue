...
function applyItem(item) {
  selected.value = item
  edit.value = { item_name: item.item_name, description: item.lc_description || '', low_stock: item.lc_low_stock || 0, sold_out: !!item.lc_sold_out, archived: !!item.disabled, price: item.price ?? '', selling_options: (item.selling_options || []).map(row => ({ ...row })) }
}
