...
const props = defineProps({ shop: { type: String, required: true }, editable: Boolean, fishShop: Boolean })
...
const showSetup = ref(false), showCreate = ref(false), form = ref({ item_name: '', item_group: props.fishShop ? 'Fish' : '', stock_uom: props.fishShop ? 'Kg' : '' })
const deleteConfirm = ref(false)
const selected = ref(null), edit = ref({}), panel = ref('details'), editor = ref(null)
const imageFile = ref(null), imageInput = ref(null)
...
function selectFishUnit(unit) { form.value.stock_uom = unit; fishPack.value = false; pieces.value = ''; approximateWeight.value = '' }
...
function applyItem(item) {
  selected.value = item
  edit.value = { item_name: item.item_name, description: item.lc_description || '', low_stock: item.lc_low_stock || 0, sold_out: !!item.lc_sold_out, archived: !!item.disabled, price: item.price ?? '', ...(item.fish_inventory ? { validity_hours: item.validity_hours || '' } : {}), selling_options: (item.selling_options || []).map(row => ({ ...row })) }
}
...
async function uploadImage() {
  if (!imageFile.value) { error.value = 'Choose a JPG, PNG or WebP image first.'; return }
  if (!['image/jpeg', 'image/png', 'image/webp'].includes(imageFile.value.type) || imageFile.value.size > 5 * 1024 * 1024) { error.value = 'Choose a JPG, PNG or WebP image up to 5 MB.'; return }
  saving.value = true; error.value = ''
  try {
    applyItem(await upload('owner.upload_product_image', { shop: props.shop, item: selected.value.name }, imageFile.value))
    imageFile.value = null
    if (imageInput.value) imageInput.value.value = ''
    await load(); success.value = 'Product image updated.'
  } catch (e) { error.value = e.message }
  finally { saving.value = false }
}
async function open(item) {
  ...
  try {
    ...
    adjustment.value = { action: 'Add', quantity: '', unit_cost: '', reason: '', ...(selected.value.fish_inventory ? { validity_hours: selected.value.validity_hours || '' } : {}) }
...
async function deleteUnused() {
  ...
  try {
    ...
    await call('products.delete_unused', { shop: props.shop, item: item.name, modified: item.modified, confirmation: item.name }, true)
    editor.value.close(); selected.value = null; deleteConfirm.value = false
    await load(true); success.value = 'Unused product deleted.'
  } catch (e) { error.value = e.message }
  finally { saving.value = false }
}
...
function close(event) { if (saving.value || pending.value) event?.preventDefault(); else { imageFile.value = null; editor.value.close() } }
onMounted(init)
</script>
...
<template>
  <section class="products-panel">
    ...
    <form v-if="showCreate && editable" class="inventory-form" @submit.prevent="create">
      <h3>{{ fishShop ? 'Add a fish product' : 'A new addition to your shelves' }}</h3><template v-if="fishShop"><div class="fish-selling-mode" role="group" aria-label="How will this fish be sold?"><button type="button" :class="{ selected: form.stock_uom === 'Kg' }" :aria-pressed="form.stock_uom === 'Kg'" :disabled="saving" @click="selectFishUnit('Kg')"><strong>By weight</strong><span>Stock in kg · price per kg</span></button><button type="button" :class="{ selected: form.stock_uom === 'Nos' }" :aria-pressed="form.stock_uom === 'Nos'" :disabled="saving" @click="selectFishUnit('Nos')"><strong>By pieces</strong><span>Stock in pieces · price per piece</span></button></div><p class="muted">{{ form.stock_uom === 'Nos' ? 'Receive 50 pieces, sell 3, and keep 47. Set a price per piece: ₹150 each means 3 pieces cost ₹450. No weighing or later price adjustment.' : 'Receive and sell kilograms. Set weighed pack options such as 0.500, 0.530 or 0.600 kg.' }} Each product has its own stock. Split your received fish between products without counting it twice.</p></template><div class="form-columns"><label>Product name<input v-model="form.item_name" required maxlength="140" :placeholder="fishShop ? (form.stock_uom === 'Nos' ? 'e.g. Mackerel — Pieces' : 'e.g. Mackerel — Weight') : 'e.g. Whole wheat bread'" :disabled="saving"></label><label>Category<select v-model="form.item_group" required :disabled="saving"><option value="">Select category</option><option v-for="g in choices.groups" :key="g">{{ g }}</option></select></label><label v-if="!fishShop">Unit<select v-model="form.stock_uom" required :disabled="saving"><option value="">Select unit</option><option v-for="u in choices.uoms" :key="u">{{ u }}</option></select></label></div><template v-if="fishShop && form.stock_uom === 'Kg'"><label class="check-label"><input v-model="fishPack" type="checkbox" :disabled="saving">Sell as a fish pack</label><div v-if="fishPack" class="form-columns"><label>Number of pieces<input v-model="pieces" type="number" min="1" step="1" placeholder="6" required :disabled="saving"></label><label>Approx. pack weight (kg)<input v-model="approximateWeight" type="number" min="0.000001" step="any" placeholder="0.5" required :disabled="saving"></label></div><p v-if="fishPack" class="muted">This is one pack, not stock received. Set your price per kg after creating it. The final bill uses the actual packed weight.</p></template><button class="lc-primary" :disabled="saving">{{ saving ? 'Creating…' : 'Create product' }}</button>
    </form>
    ...
    <div v-else class="inventory-table-wrap"><table class="inventory-table"><thead><tr><th>Product</th><th>Selling price</th><th>Inventory</th><th>Status</th><th><span class="sr-only">Actions</span></th></tr></thead><tbody><tr v-for="item in items" :key="item.name"><td><div class="product-identity"><span class="product-letter"><img v-if="item.image" :src="item.image" :alt="item.item_name" loading="lazy" decoding="async" @error="item.image = ''"><span v-else aria-hidden="true">{{ item.item_name.slice(0, 1).toUpperCase() }}</span></span><div><strong>{{ item.item_name }}</strong><small>{{ item.item_group }} · {{ item.stock_uom }}</small></div></div></td><td data-label="Selling price">{{ money(item.price, item.currency) }}<small>per {{ item.stock_uom }}</small></td><td data-label="Inventory"><strong>{{ item.stock.available }}</strong> {{ item.stock_uom }} <small>sellable</small><small>{{ item.stock.actual }} on hand · {{ item.stock.reserved }} reserved<span v-if="item.stock.order_reserved"> ({{ item.stock.order_reserved }} in new orders)</span></small><span v-if="item.stock.available <= Number(item.lc_low_stock || 0) && !item.disabled" class="low-stock">Low stock</span></td><td data-label="Status"><span class="inventory-badge" :class="{ unavailable: item.availability !== 'In stock' }">{{ item.availability }}</span></td><td><button :disabled="saving" @click="open(item)">{{ editable ? 'Manage →' : 'View →' }}</button></td></tr></tbody></table></div>
    ...
    <dialog ref="editor" class="product-dialog" aria-labelledby="product-dialog-title" @cancel="close">
      <template v-if="selected">
        ...
        <form v-if="panel === 'details'" @submit.prevent="saveProduct"><fieldset :disabled="!editable || saving"><div class="product-image-editor"><div class="product-image-preview"><img v-if="selected.image" :src="selected.image" :alt="selected.item_name" @error="selected.image = ''"><span v-else aria-hidden="true">{{ selected.item_name.slice(0, 1).toUpperCase() }}</span></div><div><strong>Product photo</strong><small>Shown on the storefront and in the cart. JPG, PNG or WebP up to 5 MB.</small><input ref="imageInput" type="file" accept="image/jpeg,image/png,image/webp" @change="chooseImage"><button type="button" :disabled="!imageFile || saving" @click="uploadImage">{{ saving ? 'Uploading…' : 'Upload photo' }}</button></div></div><label>Product name<input v-model="edit.item_name" required maxlength="140"></label><label>Description<textarea v-model="edit.description" maxlength="5000" rows="3"></textarea></label><div class="form-columns"><label>Selling price per {{ selected.stock_uom === 'Nos' ? 'piece' : selected.stock_uom }} ({{ selected.currency }})<input v-model="edit.price" type="number" min="0" step="0.000001" placeholder="Not priced"></label><label>Low-stock threshold ({{ selected.stock_uom === 'Nos' ? 'pieces' : selected.stock_uom }})<input v-model="edit.low_stock" type="number" min="0" step="0.000001" required></label></div><p v-if="selected.fish_inventory && selected.stock_uom === 'Nos'" class="muted">Fixed price per piece. Customers choose how many pieces they want; size and weight may vary. New prices apply to future orders only. Stock and receipt costs are per piece, not per kg.</p><label v-if="selected.fish_inventory">Default stock validity (hours)<input v-model="edit.validity_hours" type="number" min="0.000001" max="8760" step="any" required><small>Applies to new receipts only. Existing lots keep their original expiry.</small></label><SellingOptionsEditor v-if="selected.stock_uom === 'Kg'" v-model="edit.selling_options" :uom="selected.stock_uom" :disabled="!editable || saving" /><label class="check-label"><input v-model="edit.sold_out" type="checkbox"><span>Mark as sold out<small>Pause availability without changing physical stock.</small></span></label><label class="check-label"><input v-model="edit.archived" type="checkbox"><span>Archive product<small>Disable the item; preserve stock and transaction history. Uncheck to restore.</small></span></label><p class="muted">Stock unit stays {{ selected.stock_uom }}. Taxes follow the shop’s checkout configuration.</p><button v-if="editable" class="lc-primary" :disabled="saving">{{ saving ? 'Saving…' : 'Save product' }}</button><section v-if="session.platform_admin" class="product-delete-panel"><h3>Delete an unused product</h3><p>Only products without stock, transactions or linked records can be deleted. Otherwise, use Archive product.</p><button v-if="!deleteConfirm" type="button" class="fish-waste-button" :disabled="saving || !!pending" @click="deleteConfirm = true">Delete unused product</button><div v-else role="alert"><p>Permanently delete <strong>{{ selected.item_name }}</strong>? This removes the product from your catalog.</p><div class="button-row"><button type="button" :disabled="saving" @click="deleteConfirm = false">Keep product</button><button type="button" class="fish-waste-button" :disabled="saving || !!pending" @click="deleteUnused">{{ saving ? 'Deleting…' : 'Yes, delete product' }}</button></div></div></section></fieldset></form>
        <form v-else-if="panel === 'stock'" @submit.prevent="moveStock"><div class="stock-total"><span>SELLABLE IN {{ selected.warehouse || 'UNCONFIGURED WAREHOUSE' }}</span><strong>{{ selected.stock.available }} <small>{{ selected.stock_uom }}</small></strong><small>{{ selected.stock.actual }} on hand · {{ selected.stock.reserved }} reserved<span v-if="selected.stock.order_reserved"> · {{ selected.stock.order_reserved }} awaiting shop response</span></small></div><p class="muted">{{ selected.fish_inventory ? 'Receive fresh fish or remove unexpired stock. Use Fish inventory to mark expired lots as wastage.' : 'Record received stock or remove damaged, wasted or missing stock. Sales must go through orders and invoices.' }}</p><fieldset :disabled="!editable || saving || !!pending"><label>Operation<select v-model="adjustment.action"><option>Add</option><option>Remove</option></select></label><div class="form-columns"><label>Quantity ({{ selected.stock_uom }})<input v-model="adjustment.quantity" type="number" :min="selected.fish_inventory && selected.stock_uom === 'Nos' ? 1 : 0.000001" :step="selected.fish_inventory && selected.stock_uom === 'Nos' ? 1 : 0.000001" required></label><label v-if="adjustment.action === 'Add'">Unit cost ({{ selected.currency }})<input v-model="adjustment.unit_cost" type="number" min="0.000001" step="0.000001" required></label></div><label v-if="selected.fish_inventory && adjustment.action === 'Add'">Stock validity (hours)<input v-model="adjustment.validity_hours" type="number" min="0.000001" max="8760" step="any" required><small>Applies only to this new receipt.</small></label><label>Reason<textarea v-model="adjustment.reason" required minlength="3" maxlength="500" placeholder="e.g. Received stock, damaged packaging or stock count shortage"></textarea></label></fieldset><p v-if="pending" class="lc-notice">The result is not confirmed. Retry this same adjustment before starting another one.</p><button v-if="editable" class="lc-primary" :disabled="saving || !configured">{{ saving ? 'Recording…' : pending ? 'Retry same adjustment' : 'Record stock adjustment' }}</button></form>
