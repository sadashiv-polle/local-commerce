...
const expiringLots = computed(() => (data.value?.lots || []).filter(row => !row.expired && row.expires_at && new Date(String(row.expires_at).replace(' ', 'T')).getTime() - Date.now() <= 24 * 60 * 60 * 1000))
const expiredLots = computed(() => (data.value?.lots || []).filter(row => row.expired && Number(row.free || 0) > 0))
function reviewLots(expired = false) {
  section.value = 'stock'
  nextTick(() => document.querySelector(expired ? '.fish-lot-card.expired' : '.fish-lot-card:not(.expired)')?.scrollIntoView({ behavior: 'smooth', block: 'start' }))
}
...
const totals = computed(() => {
  ...
  for (const row of data.value?.lots || []) {
    const sum = result[units.value[row.item]]
    if (!sum) continue
    sum[row.expired ? 'expired' : 'fresh'] += row.free
    sum.reserved += row.reserved
  }
  return result
})
...
<template>
  <section class="fish-workspace">
    ...
    <template v-if="data">
      <div v-if="section === 'stock' && (expiringLots.length || expiredLots.length)" class="stock-expiry-alert" role="status"><div><strong>{{ expiredLots.length ? 'Stock needs attention' : 'Stock expires soon' }}</strong><p v-if="expiredLots.length">{{ expiredLots.length }} lot{{ expiredLots.length === 1 ? '' : 's' }} already expired. Review and record free stock as wastage.</p><p v-else>{{ expiringLots.length }} lot{{ expiringLots.length === 1 ? '' : 's' }} expires within 24 hours.</p></div><div class="button-row"><button v-if="expiredLots.length" type="button" class="fish-waste-button" @click="reviewLots(true)">Review expired stock</button><button v-if="expiringLots.length" type="button" @click="reviewLots(false)">Review expiring stock</button></div></div>
      <div v-if="section === 'stock'" class="fish-stock-section"><div class="inventory-meaning"><strong>How to read stock</strong><span><b>Sellable</b> can be ordered now.</span><span><b>On hand</b> is the physical total.</span><span><b>Reserved</b> is held for orders.</span><span><b>Expired</b> needs wastage review.</span></div>
        ...
        <article v-for="lot in data.lots" :key="lot.name" class="fish-lot-card" :class="{ expired: lot.expired }"><div><strong>{{ names[lot.item] }}</strong><span class="inventory-badge" :class="{ unavailable: lot.expired }">{{ lot.expired ? 'Expired' : 'Valid' }}</span><p>{{ kg(lot.remaining) }} {{ unit(lot.item) }} remaining · {{ kg(lot.reserved) }} {{ unit(lot.item) }} reserved</p><small>{{ lot.expired ? 'Expired' : 'Valid until' }} {{ when(lot.expires_at) }} · Receipt {{ when(lot.creation) }}</small><small>{{ lot.opening ? 'Tracked existing stock' : lot.stock_entry }} · Receipt cost {{ money(lot.unit_cost) }} / {{ perUnit(lot.item) }}</small></div><button v-if="lot.expired" class="fish-waste-button" :disabled="busy || !!pending || lot.free <= 0 || !data.wastage_account" @click="open(data.items.find(item => item.name === lot.item), 'Wastage', lot)">Mark {{ kg(lot.free) }} {{ unit(lot.item) }} as wastage</button></article>
      ...
      <div v-if="section === 'prices'"><p class="muted">Set today's market price per kg and per piece. Existing orders retain their original prices.</p><article v-for="item in data.items" :key="item.name" class="fish-product-card"><div><h3>{{ item.item_name }}</h3><strong>{{ money(item.price) }} / {{ perUnit(item.name) }}</strong><small>{{ item.stock_uom === 'Nos' ? 'Price per piece · no weighing needed' : `${item.selling_options.length} weight / pack options` }}</small></div><button :disabled="busy || !!pending" @click="open(item, 'Price')">Edit market prices</button></article><h3>Price history</h3><article v-for="row in data.price_history" :key="row.name" class="history-row"><strong>{{ names[row.item] }} · {{ money(row.weight_price) }} / {{ perUnit(row.item) }}</strong><small>{{ when(row.creation) }} · {{ row.owner }}</small><small v-for="offer in JSON.parse(row.selling_options_json || '[]')" :key="offer.id">{{ offer.label }} · {{ offer.billing === 'Pieces' ? `${money(offer.piece_price)} / piece` : 'Price by packed weight' }} · {{ offer.enabled === false ? 'Hidden' : 'Shown' }}</small></article></div>
      <div v-if="section === 'movements'"><p class="muted">Latest 100 movements. Sales leave stock when picked up; revenue is recorded when invoiced.</p><article v-for="row in data.movements" :key="row.name" class="history-row"><strong>{{ names[row.item] }} · {{ row.kind }} · {{ ['Receipt', 'Opening'].includes(row.kind) ? '+' : '−' }}{{ kg(row.quantity) }} {{ unit(row.item) }}</strong><p>{{ row.reason }}</p><small>{{ when(row.creation) }} · {{ row.stock_entry || row.delivery_note || 'Existing stock tracking' }}</small></article></div>
      <section v-if="section === 'reports'" class="fish-reports"><form class="fish-report-dates" @submit.prevent="loadReport"><label>From<input v-model="startDate" type="date" required></label><label>Through<input v-model="endDate" type="date" required></label><button class="lc-primary" :disabled="loading">Show report</button></form><template v-if="report"><div class="inventory-metrics"><div><small>PRODUCT REVENUE</small><strong>{{ money(report.totals.revenue) }}</strong></div><div><small>COST OF INVOICED FISH</small><strong>{{ money(report.totals.cost) }}</strong></div><div><small>WASTAGE COST</small><strong class="fish-danger">{{ money(report.totals.wastage_cost) }}</strong></div><div><small>PROFIT AFTER STOCK LOSSES</small><strong>{{ money(report.totals.profit_after_stock_losses) }}</strong></div></div><p class="muted">{{ report.basis }}</p><p>Other stock removal cost: {{ money(report.totals.removal_cost) }} · Delivery revenue: {{ money(report.totals.delivery_revenue) }} · Closing stock value: {{ money(report.totals.closing_value) }}</p><div class="fish-report-table"><table><thead><tr><th>Fish</th><th>Unit</th><th>Opening</th><th>In</th><th>Out</th><th>Closing</th><th>Sold</th><th>Sold pieces</th><th>Revenue</th><th>Sales cost</th><th>Wasted</th><th>Waste cost</th><th>Removal cost</th><th>Profit after stock losses</th></tr></thead><tbody><tr v-for="row in report.items" :key="row.item"><th>{{ row.item_name }}</th><td>{{ row.stock_uom === 'Nos' ? 'Pieces' : 'Kg' }}</td><td>{{ kg(row.opening_quantity) }}</td><td>{{ kg(row.stock_in_quantity) }}</td><td>{{ kg(row.stock_out_quantity) }}</td><td>{{ kg(row.closing_quantity) }}</td><td>{{ kg(row.sold_quantity) }}</td><td>{{ kg(row.sold_pieces) }}</td><td>{{ money(row.revenue) }}</td><td>{{ money(row.cost) }}</td><td>{{ kg(row.wastage_quantity) }}</td><td>{{ money(row.wastage_cost) }}</td><td>{{ money(row.removal_cost) }}</td><td>{{ money(row.profit_after_stock_losses) }}</td></tr></tbody></table></div></template></section>
    </template>
    <dialog ref="modal" class="product-editor fish-editor"><template v-if="selected"><div class="product-editor-heading"><div><span class="eyebrow">{{ operation === 'Price' ? 'DAILY MARKET VALUE' : 'FISH STOCK' }}</span><h2>{{ selected.item_name }}</h2></div><button type="button" aria-label="Close editor" @click="modal.close()">×</button></div><p v-if="error" class="lc-notice" role="alert">{{ error }}</p><form v-if="operation === 'Price'" @submit.prevent="savePrice"><fieldset :disabled="busy"><label>Price per {{ perUnit(selected.name) }} ({{ data.currency }})<input v-model="form.price" type="number" min="0.01" step="0.01" required></label><label>Default validity for new receipts (hours)<input v-model="form.validity_hours" type="number" min="0.000001" max="8760" step="any" required></label><SellingOptionsEditor v-if="selected.stock_uom === 'Kg'" v-model="form.selling_options" uom="Kg" :disabled="busy" /><button class="lc-primary">Save market prices</button></fieldset></form><form v-else @submit.prevent="saveStock"><p v-if="operation === 'Opening'" class="muted">Track the existing {{ kg(selected.stock.untracked) }} {{ unit(selected.name) }} in your warehouse. This does not receive stock again or create another accounting entry. Choose its remaining validity carefully.</p><p v-if="operation === 'Wastage'" class="lc-notice">This posts a stock issue to the configured wastage expense account. It preserves the stock and accounting history.</p><fieldset :disabled="busy || !!pending"><label v-if="operation !== 'Opening'">{{ operation }} quantity ({{ unit(selected.name) }})<input v-model="form.quantity" type="number" :min="selected.stock_uom === 'Nos' ? 1 : 0.000001" :step="selected.stock_uom === 'Nos' ? 1 : 'any'" required></label><label v-if="operation === 'Add'">Receipt cost per {{ perUnit(selected.name) }} ({{ data.currency }})<input v-model="form.unit_cost" type="number" min="0.000001" step="0.000001" required><small v-if="selected.stock_uom === 'Nos'">Total purchase cost ÷ pieces received. For example, ₹400 ÷ 50 pieces = ₹8 per piece.</small></label><label v-if="['Add', 'Opening'].includes(operation)">Stock validity from now (hours)<input v-model="form.validity_hours" type="number" min="0.000001" max="8760" step="any" required><small>Only this stock lot receives this expiry time.</small></label><label v-if="operation !== 'Opening'">Reason<textarea v-model="form.reason" minlength="3" maxlength="500" required /></label><button :class="operation === 'Wastage' ? 'fish-waste-button' : 'lc-primary'">{{ busy ? 'Recording…' : operation === 'Opening' ? 'Track existing stock' : operation === 'Wastage' ? 'Confirm wastage' : 'Record stock movement' }}</button></fieldset></form></template></dialog>
  </section>
</template>
