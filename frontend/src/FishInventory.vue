...
<template>
  <section class="fish-workspace">
    ...
    <dialog ref="modal" class="product-editor fish-editor"><template v-if="selected"><div class="product-editor-heading"><div><span class="eyebrow">{{ operation === 'Price' ? 'DAILY MARKET VALUE' : 'FISH STOCK' }}</span><h2>{{ selected.item_name }}</h2></div><button type="button" aria-label="Close editor" @click="modal.close()">×</button></div><p v-if="error" class="lc-notice" role="alert">{{ error }}</p><form v-if="operation === 'Price'" @submit.prevent="savePrice"><fieldset :disabled="busy"><label>Price per kg ({{ data.currency }})<input v-model="form.price" type="number" min="0.01" step="0.01" required></label><label>Default validity for new receipts (hours)<input v-model="form.validity_hours" type="number" min="0.000001" max="8760" step="any" required></label><SellingOptionsEditor v-model="form.selling_options" uom="Kg" :disabled="busy" /><button class="lc-primary">Save market prices</button></fieldset></form><form v-else @submit.prevent="saveStock"><p v-if="operation === 'Opening'" class="muted">Track the existing {{ kg(selected.stock.untracked) }} kg in your warehouse. This does not receive stock again or create another accounting entry. Choose its remaining validity carefully.</p><p v-if="operation === 'Wastage'" class="lc-notice">This posts a stock issue to the configured wastage expense account. It preserves the stock and accounting history.</p><fieldset :disabled="busy || !!pending"><label v-if="operation !== 'Opening'">{{ operation }} quantity (kg)<input v-model="form.quantity" type="number" min="0.000001" step="any" required></label><label v-if="operation === 'Add'">Receipt cost per kg ({{ data.currency }})<input v-model="form.unit_cost" type="number" min="0.01" step="0.01" required></label><label v-if="['Add', 'Opening'].includes(operation)">Stock validity from now (hours)<input v-model="form.validity_hours" type="number" min="0.000001" max="8760" step="any" required><small>Only this stock lot receives this expiry time.</small></label><label v-if="operation !== 'Opening'">Reason<textarea v-model="form.reason" minlength="3" maxlength="500" required /></label><button :class="operation === 'Wastage' ? 'fish-waste-button' : 'lc-primary'">{{ busy ? 'Recording…' : operation === 'Opening' ? 'Track existing stock' : operation === 'Wastage' ? 'Confirm wastage' : 'Record stock movement' }}</button></fieldset></form></template></dialog>
  </section>
</template>
