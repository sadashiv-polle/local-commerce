...
<template>
  <section class="sales-dashboard">
    ...
    <template v-if="data">
      <div class="dashboard-metrics"><article class="dashboard-sales"><small>DELIVERED SALES</small><strong>{{ money(data.sales, data.currency) }}</strong><span>{{ data.delivered_orders }} delivered orders · since {{ data.since }}</span></article><article><small>ORDERS RECEIVED</small><strong>{{ data.received_orders }}</strong><span>In the selected period</span></article><article><small>NEEDS YOUR RESPONSE</small><strong>{{ data.pending_orders }}</strong><button @click="emit('open', 'orders')">Review orders →</button></article><article><small>IN PROGRESS</small><strong>{{ data.active_orders }}</strong><span>All current active orders</span></article></div><p class="dashboard-explanation">Sales include taxes and delivery charges on delivered orders. Stock alerts and pending cash show the current position across all dates.</p>
